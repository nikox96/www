    ...
    app.get('/new-order-product', isLoggedIn, function (req, res) {
        Appoggio.find(req.user.cage, '', function (appErr, appRes) {
            if (appErr) {
                ...
                Product.list(req.query.ccodda, req.query.ccoda, req.query.sven, req.query.sgrp, req.query.xprod, appRes[0].idord, function (productErr, productRes) {
                    if (productErr) {
        ...
        Appoggio.find(req.user.cage, '', function (appErr, appRes) {
            if (appErr) {
                ...
                Order.newOrderProduct(appRes[0].idord, req.body.ccodprod, req.body.iqta, req.body.psco, '', function (orderErr, orderRes) {
                    ...
                    Product.list((req.body.ccoddap && req.body.ccoddap > 0 ? req.body.ccoddap : 0), (req.body.ccodap && req.body.ccodap > 0 ? req.body.ccodap : 999999), (req.body.svenp && req.body.svenp !== '' ? req.body.svenp : ''), (req.body.sgrpp && req.body.sgrpp !== '' ? req.body.sgrpp : ''), (req.body.xprodp && req.body.xprodp !== '' ? req.body.xprodp : ''), appRes[0].idord, function (productErr, productRes) {
                        if (productErr) {
                            req.flash('orderMessage', 'Nessun cliente trovato');
                        } else {
                ...
                Order.findCamp(req.query.ccod, function (ordCampErr, ordCampRes) {
                    Client.find(ordRes.ccli, function (cliErr, cliRes) {
                        if (cliErr)
                            ...
                            Agent.find(cliRes.cage, function (ageErr, ageRes) {
                                Order.findProduct(req.query.ccod, function (righeErr, righeRes) {
        ...
        Order.getUserOrder(req.user.cage, null, null, null, function (ordErr, ordRes) {
            if (ordErr) {
                ...
                for (i = 0; i < ordRes.length; i++) {
                    ...
                    order.iimp = (+ordRes[i].iimp).toFixed(2);
                    order.data = ordRes[i].data;
    ...
    app.get('/get-csv-client', isLoggedIn, function (req, res) {
        ...
        Client.list(req.query.ccod, null, null, function (cliErr, cliRes) {
            if (cliErr) {
                ...
                xrig += cliRes[0].czona + ';';
                xrig += cliRes[0].xcli1 + ';';
                xrig += (cliRes[0].xcli2 ? cliRes[0].xcli2 : '') + ';importato da portale;';
                xrig += '01;';
                xrig += cliRes[0].psco + ';';
                xrig += cliRes[0].csdi + ';';
            ...
            if (cliErr) {
                ...
                Order.getSumCtv(cliRes[0].ccod, function (getSumCtvErr, getSumCtvRes) {
                    res.render('detail-client.ejs', {
                        ...
                        psco: cliRes[0].psco,
                        csdi: cliRes[0].csdi,
                        sumCtvOrd: (getSumCtvErr ? 0 : (+getSumCtvRes.sumctv).toFixed(2)),
                        user: req.user
                    });
                });
            }
                    ...
                    if (appErr) {
                        ...
                        Order.updateCcli(appRes[0].idord, req.body.ccod, function (ordErr, ordRes) {
                            if (ordErr) {
                                ...
                                Product.list(0, 999998, '', '', '', appRes[0].idord, function (productErr, productRes) {
                                    if (productErr) {
                                        req.flash('orderMessage', 'Nessun prodotto trovato');
                                    }
                                    if (productRes) {
            ...
            if (righe[i].ccodprod) {
                ...
                Product.find(righe[i].ccodprod, function (prodErr, prodRes) {
                    if (prodErr) {
                        ...
                        rig.qExpUmSec = '';
                        rig.qExpUmMag = '';
                        rig.iprz = (product.psco === 100 ? '0' : prodRes.iprz);
                        rig.sco1 = (product.psco === 100 ? '0' : product.psco);
                        rig.fscoAna = '';
                        rig.sco2 = '';
                        rig.sco3 = '';
                        rig.scoUni = '';
                        rig.provvAge = (cliente && cliente.percprovv ? cliente.percprovv : (agente && agente.percprovv ? +agente.percprovv - (cliente.contrattista ? 2 : 0) : ''));
                        rig.impVal = (product.psco === 100 ? '0' : product.iimp);
    ...
    if (j < cliRes.length) {
        ...
        Client.getContr(clienteContr.ccod, function (getContrErr, getContrRes) {
            if (getContrErr) {
                ...
                clienteContr.contrctv = parseFloat((getContrRes.length > 0 ? getContrRes[0].iimp : 0)).toFixed(2);
                clienteContr.contrmen = parseFloat((getContrRes.length > 0 ? getContrRes[0].iimp : 0) / 12).toFixed(2);
            }
            Order.getSumCtv(clienteContr.ccod, function (getSumCtvErr, getSumCtvRes) {
                if (getSumCtvErr) {
                    ...
                    clienteContr.contrres = parseFloat(getSumCtvRes.sumctv).toFixed(2);
                    clienteContr.ctvord = parseFloat(+clienteContr.contrctv - clienteContr.contrres).toFixed(2);
                }
