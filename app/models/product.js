...
Product.list = function list(ccodda, ccoda, sven, xgrp, xprod, idord, callback) {
    ...
    var query = "select z.* from (select a.*, CASE WHEN b.iqta IS NULL THEN 0 ELSE b.iqta END AS iqta, CASE WHEN b.psco IS NULL THEN 0 ELSE b.psco END AS psco, CASE WHEN b.descrpromo IS NULL THEN '' ELSE b.descrpromo END AS descrpromo from portale.prodotti a left outer join (select ccodprod, iqta, psco, descrpromo from portale.righe_ordini where ccod = $6 AND ccodprod <> 999999) b on a.ccod=b.ccodprod WHERE (a.ccod >= $1 AND a.ccod <= $2 AND a.ccod <> 999999 AND a.xgrp ILIKE $3 AND a.xdesc ILIKE $4 AND a.sven ILIKE $5 AND a.sval = true) OR b. iqta IS NOT NULL) z ORDER BY z.iqta desc, z.ccod asc, z.psco desc;";
    console.log(query);
    db.query(query
